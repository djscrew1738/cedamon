...
PORT_RULES: list[HeuristicRule] = [
    ...
    HeuristicRule(
        id="port-5432-postgres-nuclei",
        name="PostgreSQL port open — check for misconfig and CVEs",
        category="port",
        priority=2,
        tool_name="execute_nuclei",
        rationale="PostgreSQL may expose unauth access or known vulns; run targeted templates.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.2,
        suggested_args={"templates": ["postgres/", "database/"]},
    ),
    HeuristicRule(
        id="port-27017-mongo-nuclei",
        name="MongoDB port open — check for unauth access",
        category="port",
        priority=1,
        tool_name="execute_nuclei",
        rationale="MongoDB without auth is a critical data exposure; test immediately.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["mongodb/", "database/"]},
    ),
    HeuristicRule(
        id="port-6379-redis-nuclei",
        name="Redis port open — check for unauth access",
        category="port",
        priority=1,
        tool_name="execute_nuclei",
        rationale="Redis without auth allows full data access and potential RCE via module loading.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["redis/", "database/"]},
    ),
    HeuristicRule(
        id="port-1521-oracle",
        name="Oracle DB port open — enumerate SID and test creds",
        category="port",
        priority=2,
        tool_name="execute_nuclei",
        rationale="Oracle databases often have default SIDs and accounts; enumerate before brute force.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["oracle/", "database/"]},
    ),
    HeuristicRule(
        id="port-1433-mssql",
        name="MSSQL port open — probe for NTLM relay and default creds",
        category="port",
        priority=2,
        tool_name="execute_nuclei",
        rationale="MSSQL exposes NTLM auth and may allow xp_cmdshell if creds are guessed.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["mssql/", "database/"]},
    ),
    HeuristicRule(
        id="port-5900-vnc",
        name="VNC port open — check for no-auth or weak auth",
        category="port",
        priority=2,
        tool_name="execute_nuclei",
        rationale="VNC without authentication gives full remote desktop access.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["vnc/"]},
    ),
    HeuristicRule(
        id="port-10000-webmin",
        name="Webmin port open — check for CVEs and default creds",
        category="port",
        priority=1,
        tool_name="execute_nuclei",
        rationale="Webmin is a common attack vector with critical CVEs (CVE-2019-15107).",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.4,
        suggested_args={"templates": ["webmin/", "cves/"]},
    ),
    HeuristicRule(
        id="port-1883-mqtt",
        name="MQTT port open — probe for unauthenticated broker",
        category="port",
        priority=3,
        tool_name="execute_curl",
        rationale="MQTT brokers without auth leak IoT telemetry and topic structures.",
        phase="informational",
        task_cluster="recon",
        cost_score=0.1,
        suggested_args={"urls": ["mqtt://{{target}}:1883"]},
    ),
]
...
TECH_KEYWORDS: dict[str, str | set[str]] = {
    ...
    "kafka": {"kafka", "apache kafka", "kafka broker"},
...
TECH_RULES: list[HeuristicRule] = [
    ...
    # ── Commonly-seen production technologies (not yet in TECH_KEYWORDS) ──
    HeuristicRule(
        id="tech-postgresql",
        name="PostgreSQL detected",
        category="technology",
        priority=2,
        tool_name="execute_nuclei",
        rationale="PostgreSQL misconfigs and CVEs are common in production deployments.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.2,
        suggested_args={"templates": ["postgres/", "database/"]},
    ...
    HeuristicRule(
        ...
        priority=2,
        tool_name="execute_nuclei",
        rationale="MySQL default creds and CVEs are common; run targeted templates.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.2,
        suggested_args={"templates": ["mysql/", "database/"]},
    ),
    HeuristicRule(
        id="tech-redis",
        name="Redis detected",
        ...
        priority=1,
        tool_name="execute_nuclei",
        rationale="Redis without auth allows RCE; check immediately.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["redis/"]},
    ),
    HeuristicRule(
        id="tech-mongodb",
        ...
        priority=1,
        tool_name="execute_nuclei",
        rationale="MongoDB without auth exposes all data; check immediately.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["mongodb/", "database/"]},
    ...
    HeuristicRule(
        ...
        priority=2,
        tool_name="execute_nuclei",
        rationale="Kafka brokers without auth leak message topics and consumer group data.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.2,
        suggested_args={"templates": ["kafka/", "apache/"]},
    ),
    HeuristicRule(
        id="tech-prometheus",
        name="Prometheus detected",
        category="technology",
        priority=2,
        tool_name="execute_curl",
        rationale="Prometheus metrics endpoints expose infrastructure data; check for auth.",
        phase="informational",
        task_cluster="recon",
        cost_score=0.1,
        suggested_args={"urls": ["http://{{target}}:9090/api/v1/query?query=up"]},
        follow_up=["execute_nuclei"],
    ),
    HeuristicRule(
        id="tech-grafana",
        name="Grafana detected",
        category="technology",
        priority=2,
        tool_name="execute_nuclei",
        rationale="Grafana dashboards often leak metrics; check for default creds and path traversal.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["grafana/", "cves/"]},
    ),
    HeuristicRule(
        id="tech-kibana",
        name="Kibana detected",
        category="technology",
        priority=2,
        tool_name="execute_nuclei",
        rationale="Kibana exposes Elasticsearch data; check for unauth access and prototype pollution.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["kibana/"]},
    ...
    HeuristicRule(
        id="tech-splunk",
        name="Splunk detected",
        category="technology",
        priority=1,
        tool_name="execute_nuclei",
        rationale="Splunk has critical CVEs (CVE-2023-46214) and default admin creds.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.4,
        suggested_args={"templates": ["splunk/", "cves/"]},
    ),
    HeuristicRule(
        id="tech-jupyter",
        name="Jupyter detected",
        category="technology",
        priority=1,
        tool_name="execute_nuclei",
        rationale="Jupyter without auth allows arbitrary code execution via notebook creation.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.4,
    ...
    HeuristicRule(
        ...
        priority=1,
        tool_name="execute_nuclei",
        rationale="phpMyAdmin has a history of CVEs and default creds; direct DB access if cracked.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.4,
        suggested_args={"templates": ["phpmyadmin/", "cves/"]},
    ),
    HeuristicRule(
        id="tech-confluence",
        name="Confluence detected",
        category="technology",
        priority=1,
        tool_name="execute_nuclei",
        rationale="Confluence has critical CVEs (CVE-2023-22527) allowing RCE.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.4,
        suggested_args={"templates": ["confluence/", "cves/", "atlassian/"]},
    ),
    HeuristicRule(
        id="tech-jira",
        name="Jira detected",
        ...
        priority=2,
        tool_name="execute_nuclei",
        rationale="Jira instances leak project data and user info; check for CVEs.",
        phase="informational",
        task_cluster="vuln_scan",
        cost_score=0.3,
        suggested_args={"templates": ["jira/", "atlassian/"]},
    ),
]
...
COMBO_RULES: list[tuple[set[str], list[HeuristicRule]]] = [
    ...
    # ── Infrastructure combo patterns ──
    (
        {"mysql", "phpmyadmin"},
        [
            HeuristicRule(
                id="combo-mysql-phpmyadmin-nuclei",
                name="MySQL + phpMyAdmin — check for direct DB access",
                category="combo",
                priority=1,
                tool_name="execute_nuclei",
                rationale="phpMyAdmin paired with MySQL gives web-based DB control; test default creds and CVEs.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.5,
                suggested_args={"templates": ["phpmyadmin/", "mysql/"]},
            ),
        ],
    ),
    (
        ...
        [
            HeuristicRule(
                id="combo-postgres-django-nuclei",
                name="PostgreSQL + Django — check for admin panel and SQLi",
                category="combo",
                priority=2,
                tool_name="execute_nuclei",
                rationale="Django on PostgreSQL may expose /admin and ORM injection surfaces.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.3,
                suggested_args={"templates": ["django/", "sqli/"]},
            ),
        ],
    ),
    (
        ...
        [
            HeuristicRule(
                id="combo-prometheus-grafana-nuclei",
                name="Prometheus + Grafana — observability stack attack",
                category="combo",
                priority=1,
                tool_name="execute_nuclei",
                rationale="Grafana with Prometheus exposes metrics; check for default creds, unauth dashboards, and CVE-2021-43798 path traversal.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.4,
                suggested_args={"templates": ["grafana/", "prometheus/", "cves/"]},
            ),
        ],
    ),
    (
        ...
        [
            HeuristicRule(
                id="combo-elastic-kibana-nuclei",
                name="Elasticsearch + Kibana — ELK stack attack",
                category="combo",
                priority=1,
                tool_name="execute_nuclei",
                rationale="ELK stack without auth leaks all indexed data; check for ES unauth and Kibana RCE.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.4,
                suggested_args={"templates": ["elasticsearch/", "kibana/"]},
    ...
    (
        ...
        [
            HeuristicRule(
                id="combo-nginx-phpfpm-nuclei",
                name="nginx + PHP-FPM — check for FastCGI misconfig",
                category="combo",
                priority=2,
                tool_name="execute_nuclei",
                rationale="nginx with PHP-FPM may have FastCGI param injection or path info bypass.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.3,
                suggested_args={"templates": ["php/", "nginx/", "fpm/"]},
            ),
        ],
    ),
    (
        ...
        [
            HeuristicRule(
                id="combo-confluence-postgres-nuclei",
                name="Confluence + PostgreSQL — Atlassian stack attack",
                category="combo",
                priority=1,
                tool_name="execute_nuclei",
                rationale="Confluence on PostgreSQL has critical RCE CVEs (CVE-2023-22527); also check for unauth API.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.5,
                suggested_args={"templates": ["confluence/", "atlassian/", "cves/"]},
            ),
        ],
    ),
    (
        ...
        [
            HeuristicRule(
                id="combo-jira-confluence-ci",
                name="Jira + Confluence — full Atlassian suite attack",
                category="combo",
                priority=1,
                tool_name="execute_nuclei",
                rationale="Atlassian suite exposes project data, user directories, and SSO tokens; test all endpoints.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.5,
                suggested_args={"templates": ["jira/", "confluence/", "atlassian/"]},
            ),
        ],
    ),
    (
        ...
        [
            HeuristicRule(
                id="combo-redis-nodejs-nuclei",
                name="Redis + Node.js — session store attack surface",
                category="combo",
                priority=2,
                tool_name="execute_nuclei",
                rationale="Node.js using Redis for sessions may expose session data; check Redis unauth and SSRF.",
                phase="informational",
                task_cluster="vuln_scan",
                cost_score=0.3,
                suggested_args={"templates": ["redis/", "ssrf/"]},
            ),
        ],
    ),
]
